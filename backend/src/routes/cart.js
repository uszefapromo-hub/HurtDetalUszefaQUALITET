...
/**
 * Cart routes – marketplace model (shop_product_id based).
 *
 * GET    /api/cart                   – get active cart with items
 * POST   /api/cart                   – add item via shop_product_id
 * DELETE /api/cart/items/:itemId     – remove a cart item by its UUID
 * DELETE /api/cart                   – clear the entire active cart
 */
...
// ─── Helper: build items array for a cart ─────────────────────────────────────
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    return res.json({
      cart: { ...cart, items, subtotal: parseFloat(subtotal.toFixed(2)) },
    });
  } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // Validate that the shop_product exists, is active, and has sufficient stock
      ...
      if (existingItem.rows.length > 0) {
        ...
        const unitPrice = parseFloat(sp.selling_price) > 0
          ? sp.selling_price
          : sp.price_override || 0;
        await db.query(
          `INSERT INTO cart_items (id, cart_id, product_id, shop_product_id, quantity, unit_price, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, NOW())`,
          [uuidv4(), cartId, sp.global_product_id, shop_product_id, quantity, unitPrice]
        );
      }
