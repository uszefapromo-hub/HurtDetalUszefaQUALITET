...
//
// This endpoint receives signed webhook notifications from a payment provider.
// The provider must send:
//   { payment_id, status, external_ref, signature }
// where `signature` is HMAC-SHA256(WEBHOOK_SECRET, payment_id + ':' + status).
//
// Set PAYMENT_WEBHOOK_SECRET in .env.  Requests without a valid signature are
// rejected with 401.
//
// Supported status values: completed | failed | refunded
...
// Returns a redirect URL or payment token for the configured provider.
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Build provider-specific redirect / payload.
      // In production replace with real SDK calls (Stripe, P24, etc.).
 ...
 *
 * Until real integration is in place, card/p24 returns a sandbox_mode flag
 * so the frontend knows not to treat the transaction as real.
 */
function buildProviderPayload(method, payment, returnUrl) {
  ...
  if (method === 'card' || method === 'p24') {
    // Sandbox mode: no real card/P24 integration configured.
    // Set STRIPE_SECRET_KEY or P24_MERCHANT_ID in .env to enable real payments.
    ...
    // TODO: integrate real provider SDK here
    return {
      provider: method,
      payment_id: payment.id,
      redirect_url: successUrl,
