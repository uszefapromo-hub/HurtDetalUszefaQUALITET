...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const calc = () => {
    ...
    if(output.price) output.price.textContent = window.QM.money(price);
    if(output.profit) output.profit.textContent = window.QM.money(profit);
    if(output.breakEven) output.breakEven.textContent = `${breakEven} szt.`;
    if(output.totalProfit) output.totalProfit.textContent = window.QM.money(totalProfit);
    if(output.dailyAds) output.dailyAds.textContent = window.QM.money(dailyAds);
