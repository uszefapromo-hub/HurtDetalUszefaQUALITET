  ...
  function addByShopProduct(shopProductId, name, price, imgUrl, quantity){
    ...
    if(isLoggedIn()){
      return apiAddByShopProduct(shopProductId, quantity).then(function(resp){
        ...
        // The local item is stored with id = shopProductId; the API returns items by central product_id.
        // We key the mapping by shopProductId since that's what we know here.
        if(resp && resp.items){
          var newCart = getCart();
          // Find the local entry we just added (id === shopProductId)
          var local = newCart.filter(function(i){ return i.id === shopProductId; })[0];
          if(local && resp.items.length){
            // The most recently-added/updated item is last or the one whose product was just added.
            // Store all backend item IDs for this shop_product_id slot.
