...
const { loadControlConfig, saveControlConfig, getPublicControlConfig, getRuntimeValue } = require('../config/controlConfig');
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let sent = false;
      try {
        const smtpHost = getRuntimeValue('SMTP_HOST', 'zoho.smtp_host');
        if (smtpHost && nodemailer) {
          const transporter = nodemailer.createTransport({
            host: smtpHost,
            port: parseInt(getRuntimeValue('SMTP_PORT', 'zoho.smtp_port', '587'), 10),
            secure: getRuntimeValue('SMTP_SECURE', 'zoho.smtp_secure', 'false') === 'true',
            auth: {
              user: getRuntimeValue('SMTP_USER', 'zoho.smtp_user', ''),
              pass: getRuntimeValue('SMTP_PASS', 'zoho.smtp_pass', ''),
            },
          });
          await transporter.sendMail({
            from: getRuntimeValue('SMTP_FROM', 'zoho.smtp_from', getRuntimeValue('SMTP_USER', 'zoho.smtp_user', 'noreply@qualitet-market.com')),
            to,
            subject,
            text: msgBody,
          });
...
const SYSTEM_SCRIPTS = [
  ...
  { id: 'cleanup-demo-data',       name: 'Usuń stare dane katalogowe',          description: 'Usuwa stare produkty zastępcze i czyści katalog centralny przed startem',   dangerous: true,  enabled: true },
...
router.post('/scripts/:id/run', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (scriptId === 'warehouse-sync') {
      ...
      resultMessage = `Usunięto ${pResult.rows.length} starych produktów zastępczych (${spResult.rows.length} wpisów ze sklepów)`;
...
router.get('/integrations', authenticate, requireRole('owner', 'admin'), async (_req, res) => {
  try {
    return res.json({
      ok: true,
      config: getPublicControlConfig(),
      status: {
        stripe: Boolean(getRuntimeValue('STRIPE_SECRET_KEY', 'stripe.secret_key')),
        stripe_webhook: Boolean(getRuntimeValue('STRIPE_WEBHOOK_SECRET', 'stripe.webhook_secret')),
        netlify: Boolean(getRuntimeValue('NETLIFY_ACCESS_TOKEN', 'netlify.access_token') || getRuntimeValue('NETLIFY_BUILD_HOOK_URL', 'netlify.build_hook_url')),
        zoho: Boolean(getRuntimeValue('SMTP_HOST', 'zoho.smtp_host') && getRuntimeValue('SMTP_USER', 'zoho.smtp_user')),
      },
    });
  } catch (err) {
...
router.patch('/integrations', authenticate, requireRole('owner', 'admin'), async (req, res) => {
  try {
    const saved = saveControlConfig(req.body || {});
    return res.json({ ok: true, config: getPublicControlConfig(saved) });
  } catch (err) {
    console.error('admin integrations patch error:', err.message);
    return res.status(500).json({ error: 'Błąd serwera' });
  }
});

router.post('/integrations/test-mail', authenticate, requireRole('owner', 'admin'), async (req, res) => {
  const nodemailer = require('nodemailer');
  try {
    const host = getRuntimeValue('SMTP_HOST', 'zoho.smtp_host');
    const user = getRuntimeValue('SMTP_USER', 'zoho.smtp_user');
    const pass = getRuntimeValue('SMTP_PASS', 'zoho.smtp_pass');
    const from = getRuntimeValue('SMTP_FROM', 'zoho.smtp_from', user || 'noreply@qualitet-market.com');
    const secure = getRuntimeValue('SMTP_SECURE', 'zoho.smtp_secure', 'false') === 'true';
    const port = parseInt(getRuntimeValue('SMTP_PORT', 'zoho.smtp_port', secure ? '465' : '587'), 10);
    const to = req.body?.to || user;

    if (!host || !user || !pass || !to) {
      return res.status(422).json({ error: 'Brak konfiguracji SMTP lub adresu odbiorcy.' });
    }
    ...
    await transporter.sendMail({
      from,
      to,
      subject: 'QualitetMarket — test SMTP/Zoho',
      text: 'To jest test konfiguracji Zoho SMTP z panelu właściciela.',
    });

    return res.json({ ok: true, message: `Wysłano testowy e-mail do ${to}` });
  } catch (err) {
    console.error('admin integrations test-mail error:', err.message);
    return res.status(500).json({ error: err.message || 'Błąd SMTP' });
  }
});

router.post('/integrations/netlify/deploy', authenticate, requireRole('owner', 'admin'), async (_req, res) => {
  try {
    const hookUrl = getRuntimeValue('NETLIFY_BUILD_HOOK_URL', 'netlify.build_hook_url');
    if (!hookUrl) {
      return res.status(422).json({ error: 'Brak NETLIFY_BUILD_HOOK_URL / build_hook_url.' });
    }
    const response = await fetch(hookUrl, { method: 'POST' });
    const text = await response.text();
    return res.json({ ok: response.ok, status: response.status, body: text.slice(0, 1500) });
  } catch (err) {
    console.error('admin integrations netlify deploy error:', err.message);
    return res.status(500).json({ error: err.message || 'Błąd deploy hooka' });
  }
});
