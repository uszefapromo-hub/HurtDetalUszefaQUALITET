...
function getStripe() {
  ...
  const key = getRuntimeValue('STRIPE_SECRET_KEY', 'stripe.secret_key');
  if (!key) return null;
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const base = getRuntimeValue('APP_URL', 'app.app_url', 'https://www.qualitet-market.com');
      const envKey = `STRIPE_PRICE_ID_${plan.toUpperCase()}`;
      const stripePriceId = process.env[envKey] || getRuntimeValue(envKey, `stripe.price_ids.${plan}`);
