...
app.post('/api/stripe/create-session', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: 'https://qualitet-market.com/success',
      cancel_url: 'https://qualitet-market.com/koszyk',
    });

    return res.json({ url: session.url });
  } catch (error) {
    console.error('stripe create-session error:', error.message);
    return res.status(500).json({ error: 'Nie udało się utworzyć sesji Stripe' });
  }
});
