...
const { getRuntimeValue } = require('../config/controlConfig');
...
async function resolveAdminEmail() {
  const envEmail = process.env.ADMIN_EMAIL || getRuntimeValue('ADMIN_EMAIL', 'zoho.org_email');
...
async function sendImportNotification({
  ...
  try {
    ...
    const dashboardUrl = getRuntimeValue('DASHBOARD_URL', 'app.dashboard_url', 'https://www.qualitet-market.com/owner-panel.html');
